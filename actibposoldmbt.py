...
def forcedpos(posstr):
    ...
    posstr = re.sub(r'(^|\s)((?:མྱོང)་?//?)([^\s]+)', r'\1\2v.invar.myong2', posstr)
    posstr = re.sub(r'(^|\s)((?:མྱོང་བ)་?//?)([^\s]+)', r'\1\2v.invar.myong2', posstr)
    ...
    posstr = re.sub(r'(^|\s)((?:ཐུབ)་?//?)([^\s]+)', r'\1\2v.invar.thub2', posstr)
    posstr = re.sub(r'(^|\s)((?:ཐུབ་པ)་?//?)([^\s]+)', r'\1\2n.v.invar.thub2', posstr)
    ...
    posstr = re.sub(r'(^|\s)((?:དྲས་པ)་?//?)([^\s]+)', r'\1\2n.v.past.dra1', posstr)
    ...
    posstr = re.sub(r'(^|\s)((?:བཚལད)་?//?)([^\s]+)', r'\1\2v.past.tshal1', posstr)
    posstr = re.sub(r'(^|\s)((?:བཚལད་བ)་?//?)([^\s]+)', r'\1\2n.v.past.tshal1', posstr)
    posstr = re.sub(r'(^|\s)((?:བཙལ)་?//?)([^\s]+)', r'\1\2v.fut.v.past.tshal1', posstr)
    posstr = re.sub(r'(^|\s)((?:བཙལ་བ)་?//?)([^\s]+)', r'\1\2n.v.fut.n.v.past.tshal1', posstr)
    posstr = re.sub(r'(^|\s)((?:འཚལ)་?//?)([^\s]+)', r'\1\2v.invar.tshal2', posstr)
    posstr = re.sub(r'(^|\s)((?:འཚལ)་?//?)([^\s]+)(\s(?:དོ)་?/cv.fin)', r'\1\2v.past.tshal1\4', posstr)
    posstr = re.sub(r'(^|\s)((?:འཚལ)་?//?)([^\s]+)(\s(?:ནོ)་?/cv.fin)', r'\1\2v.fut.v.pres.tshal1\4', posstr)
...
def process(inp, pyrrhaoutp, posoutp, segoutp, input_format="txt", pipeline="seg:pos"):
    ...
    segstr = run_mbt(inputstr, 'conf/old-seg/segtrain.txt.settings')
    ...
    if pipeline == 'seg':
        return
    posstr = run_mbt(segstr, 'conf/old-pos/TibTrain5.txt.settings')
