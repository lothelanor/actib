...
def main():
    ...
    for filename in sorted(filenames, key=lambda fn: str(fn)):
        ...
        if previousdir is not None and (previousdir/"pos"/(noext+".txt")).is_file():
            ...
            continue
        POOL.apply_async(actibposoldmbt.processfiles, args=(filename, segoutfilename, posoutfilename, pyrrhaoutfilename, pipeline, "bdrc-tei"))
    ...
    for filename in sorted(filenames, key=lambda fn: str(fn)):
        ...
        if previousdir is not None and (previousdir/"seg"/(noext+".txt")).is_file():
            print("skipping "+noext)
            continue
        POOL.apply_async(actibposoldmbt.processfiles, args=(filename, segoutfilename, posoutfilename, pyrrhaoutfilename, pipeline, "txt"))
    POOL.close()
    POOL.join()
